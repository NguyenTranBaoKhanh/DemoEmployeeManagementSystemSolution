...
namespace ServerLibrary.Repositories.Implementations
{
    public class UserAccountRepository(IOptions<JwtSection> config, AppDbContext appDbContext) : IUserAccount
    {
        ...
        public async Task<LoginResponse> RefreshTokenAsync(RefreshToken token)
        {
            ...
            var updateRefeshToken = await appDbContext.RefreshTokenInfos.FirstOrDefaultAsync(_ => _.UserId == user.Id);
            if (updateRefeshToken is null) return new LoginResponse(false, "Refresh token could not be generated because user not signed in");

            updateRefeshToken.Token = refreshToken;
            await appDbContext.SaveChangesAsync();
